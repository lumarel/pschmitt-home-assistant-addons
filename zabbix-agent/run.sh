...
sed -i 's@^#\?\s\?\(Server\)=.*@\1='"${ZABBIX_SERVER}"'@' "${ZABBIX_CONFIG_FILE}"
sed -i 's@^#\?\s\?\(ServerActive\)=.*@\1='"${ZABBIX_SERVER_ACTIVE}"'@' "${ZABBIX_CONFIG_FILE}"
...
if [ -n "$ZABBIX_TLSPSK_IDENTITY" ]; then
  if [ -n "$ZABBIX_TLSPSK_SECRET" ]; then
    ...
    sed -i 's@^#\?\s\?\(TLSConnect\)=.*@\1='"${ZABBIX_TLSPSK_IDENTITY}"'@' "${ZABBIX_CONFIG_FILE}"
    sed -i 's@^#\?\s\?\(TLSConnect\)=.*@\1='"${ZABBIX_TLSPSK_SECRET_FILE}"'@' "${ZABBIX_CONFIG_FILE}"
  fi
fi
